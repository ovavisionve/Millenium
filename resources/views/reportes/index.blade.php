<x-app-layout>
    <x-slot name="header">
        <div class="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
            ...
            <a href="{{ route('dashboard') }}"><x-secondary-button type="button">Dashboard</x-secondary-button></a>
        </div>
    </x-slot>
                ...
                @if (request()->boolean('generar') && $lineas)
                        ...
                        <div class="pt-2">
                            ...
                            <div class="overflow-x-auto rounded border border-gray-200 dark:border-gray-600 text-sm">
                                <table class="min-w-full">
                                    <thead class="bg-gray-50 dark:bg-gray-700"><tr><th class="px-3 py-2 text-left">Vendedor</th><th class="px-3 py-2 text-end">Total USD</th></tr></thead>
                                    <tbody class="divide-y divide-gray-200 dark:divide-gray-600">
                                        @foreach ($resumenPorVendedor as $r)
                                            <tr><td class="px-3 py-2">{{ $r['nombre'] }}</td><td class="px-3 py-2 text-end">${{ number_format($r['total_usd'], 2) }}</td></tr>
                                        @endforeach
                                    </tbody>
                                </table>
                            </div>
                    ...
                    @if ($resumenPorZona->isNotEmpty())
                        <div class="pt-2">
                            ...
                            <div class="overflow-x-auto rounded border border-gray-200 dark:border-gray-600 text-sm">
                                <table class="min-w-full">
                                    <thead class="bg-gray-50 dark:bg-gray-700"><tr><th class="px-3 py-2 text-left">Zona</th><th class="px-3 py-2 text-end">Total USD</th></tr></thead>
                                    <tbody class="divide-y divide-gray-200 dark:divide-gray-600">
                                        @foreach ($resumenPorZona as $r)
                                            <tr><td class="px-3 py-2">{{ $r['zona'] }}</td><td class="px-3 py-2 text-end">${{ number_format($r['total_usd'], 2) }}</td></tr>
                                        @endforeach
                                    </tbody>
                                </table>
                            </div>
                    ...
                    <div class="overflow-x-auto border border-gray-200 dark:border-gray-600 rounded-md">
                        <table class="min-w-full text-sm text-left">
                            ...
                            <tbody class="divide-y divide-gray-200 dark:divide-gray-600">
                                ...
                                @empty
                                    <tr><td colspan="10" class="px-4 py-8 text-center text-gray-500">Sin líneas con esos filtros.</td></tr>
                                @endforelse
                            </tbody>
                        </table>
                    </div>
                        ...
                        <table class="min-w-full text-left">
                            ...
                            <tbody class="divide-y divide-gray-200 dark:divide-gray-600">
                                @forelse ($estadoCuentaFacturas as $f)
                                    ...
                                    <tr>
                                        <td class="px-3 py-2 font-mono text-xs"><a href="{{ route('facturas.show', $f) }}" class="text-indigo-600 dark:text-indigo-400 hover:underline">{{ $f->numero_factura ?? '#'.$f->id }}</a></td>
                                ...
                                @empty
                                    <tr><td colspan="8" class="px-4 py-6 text-center text-gray-500">Sin facturas con esos criterios.</td></tr>
                                @endforelse
                            </tbody>
                        </table>
                    </div>
