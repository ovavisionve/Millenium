                ...
                <div class="bg-white dark:bg-gray-800 shadow-sm sm:rounded-lg p-6 space-y-4">
                    ...
                    <div class="grid sm:grid-cols-3 gap-3 pt-2 text-sm border-t border-gray-200 dark:border-gray-600">
                        ...
                        <div class="rounded-md bg-gray-50 dark:bg-gray-700/50 p-3">
                            ...
                            <p class="text-lg font-semibold">{{ number_format($totales['kg'], 2) }} kg</p>
                        </div>
                    </div>
                        ...
                        <table class="min-w-full text-sm text-left">
                            ...
                            <tbody class="divide-y divide-gray-200 dark:divide-gray-600">
                                ...
                                <tr class="text-gray-900 dark:text-gray-100">
                                    ...
                                    @endphp
                                    <td class="px-3 py-2 text-end">{{ number_format((float) $linea->cantidad, $decCantidad) }} {{ $udAb }}</td>
                                    <td class="px-3 py-2 text-end">${{ number_format((float) $linea->precio_unitario, 2) }}</td>
                ...
                <div class="bg-white dark:bg-gray-800 shadow-sm sm:rounded-lg p-6 space-y-4">
                    ...
                    <form method="get" class="space-y-3">
                        ...
                        <div class="grid grid-cols-1 gap-3 sm:grid-cols-2 lg:grid-cols-4 lg:items-start">
                            ...
                            <div>
                                <x-input-label for="cuenta_cartera" value="Cartera (vencimiento)" />
                                <select id="cuenta_cartera" name="cuenta_cartera" class="mt-1 block w-full rounded-md border-gray-300 dark:border-gray-700 dark:bg-gray-900 dark:text-gray-300 shadow-sm">
                                    <option value="" @selected(request('cuenta_cartera')==='' )>Todas</option>
                                    <option value="vencidas" @selected(request('cuenta_cartera')==='vencidas' )>Vencidas</option>
                                    <option value="no_vencidas" @selected(request('cuenta_cartera')==='no_vencidas' )>No vencidas</option>
                                    <option value="{{ \App\Models\Factura::CARTERA_POR_VENCER }}" @selected(request('cuenta_cartera')===\App\Models\Factura::CARTERA_POR_VENCER)>Por vencer</option>
                                    <option value="{{ \App\Models\Factura::CARTERA_AL_DIA }}" @selected(request('cuenta_cartera')===\App\Models\Factura::CARTERA_AL_DIA)>Al día</option>
                                </select>
                            </div>
                            <div>
                                <x-input-label for="cuenta_verificacion" value="Verificación" />
                                <select id="cuenta_verificacion" name="cuenta_verificacion" class="mt-1 block w-full rounded-md border-gray-300 dark:border-gray-700 dark:bg-gray-900 dark:text-gray-300 shadow-sm">
                                    <option value="" @selected(request('cuenta_verificacion')==='' )>Todas</option>
                                    <option value="pendiente" @selected(request('cuenta_verificacion')==='pendiente' )>Pendientes</option>
                                    <option value="verificada" @selected(request('cuenta_verificacion')==='verificada' )>Verificadas</option>
                                </select>
                            </div>
                            <div>
                                <x-input-label for="cuenta_estado_pago" value="Pago (factura)" />
                                <select id="cuenta_estado_pago" name="cuenta_estado_pago" class="mt-1 block w-full rounded-md border-gray-300 dark:border-gray-700 dark:bg-gray-900 dark:text-gray-300 shadow-sm">
                                    <option value="" @selected(request('cuenta_estado_pago')==='' )>Todos</option>
                                    <option value="{{ \App\Models\Factura::ESTADO_PAGO_ABIERTA }}" @selected(request('cuenta_estado_pago')===\App\Models\Factura::ESTADO_PAGO_ABIERTA)>Factura por pagar</option>
                                    <option value="{{ \App\Models\Factura::ESTADO_PAGO_PAGADA }}" @selected(request('cuenta_estado_pago')===\App\Models\Factura::ESTADO_PAGO_PAGADA)>Pagadas</option>
                                </select>
                            </div>
                        </div>

                        <p class="text-xs text-gray-500 dark:text-gray-400 leading-relaxed">
                            <span class="font-medium text-gray-600 dark:text-gray-300">Cartera:</span>
                            “Por vencer” usa umbral de {{ \App\Models\Factura::DIAS_UMBRAL_POR_VENCER }} días.
                        </p>

                        {{-- Fila 2: opciones (sin chocar con acción) --}}
                        <div class="grid grid-cols-1 gap-3">
                            <div>
                                <x-input-label value="Opciones" />
                                <div class="mt-1 grid grid-cols-1 sm:grid-cols-3 gap-2">
                                    <label class="flex items-center gap-2 text-sm text-gray-700 dark:text-gray-300">
                                        <input type="checkbox" name="cuenta_modo_cliente" value="1" class="rounded border-gray-300 dark:border-gray-700" @checked(request()->boolean('cuenta_modo_cliente', true)) />
                                        <span class="font-medium">Modo cliente</span>
                                    </label>
                                    <label class="flex items-center gap-2 text-sm text-gray-700 dark:text-gray-300">
                                        <input type="checkbox" name="cuenta_solo_con_saldo" value="1" class="rounded border-gray-300 dark:border-gray-700" @checked(request()->boolean('cuenta_solo_con_saldo')) />
                                        <span class="font-medium">Solo con saldo</span>
                                    </label>
                                    <label class="flex items-center gap-2 text-sm text-gray-600 dark:text-gray-400">
                                        ...
                                        <span class="font-medium">Compatibilidad</span>
                                    </label>
                                </div>
                                <p class="mt-1 text-xs text-gray-500 dark:text-gray-400 leading-relaxed">
                                    <span class="font-medium">Modo cliente</span>: solo por pagar con saldo ·
                                    <span class="font-medium">Solo con saldo</span>: oculta $0 ·
                                    <span class="font-medium">Compatibilidad</span>: solo vencidas (&lt; hoy)
                                </p>
                            </div>
                        </div>

                        {{-- Fila 3: acción (debajo, separada) --}}
                        <div class="pt-1 border-t border-gray-200 dark:border-gray-700">
                            <div class="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
                                <div>
                                    <x-input-label value="Acción" />
                                    <p class="mt-1 text-xs text-gray-500 dark:text-gray-400">
                                        Elegí el cliente y aplicá filtros antes de generar.
                                    </p>
                                </div>
                                <div class="flex">
                                    <x-primary-button type="submit" class="h-11 w-full sm:w-auto justify-center">Ver estado de cuenta</x-primary-button>
                                </div>
                            </div>
                        </div>
                    </form>
                    ...
                    <p class="text-xs text-gray-500 dark:text-gray-400">
                        Recomendación: para enviar al cliente usá <strong>Modo cliente</strong> (solo facturas <strong>por pagar</strong> con <strong>saldo &gt; 0</strong>).
                        Si lo desmarcás, esta tabla pasa a ser un histórico (puede incluir pagadas o saldo 0).
                    </p>
