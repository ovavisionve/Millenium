<x-app-layout>
    <x-slot name="header">
        <div class="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
            ...
            <div class="flex flex-wrap gap-2">
                <a href="{{ route('cobranza.index') }}"><x-secondary-button class="!rounded-sm" type="button">Buscar otro</x-secondary-button></a>
                <a href="{{ route('clientes.edit', $cliente) }}"><x-secondary-button class="!rounded-sm" type="button">Cliente</x-secondary-button></a>
            </div>
        </div>
    </x-slot>
    ...
    <div class="py-6">
        <div class="max-w-6xl mx-auto sm:px-6 lg:px-8 space-y-4">
            @if (session('status'))
            <div class="rounded-sm bg-green-50 p-4 text-sm text-green-800 dark:bg-green-900/20 dark:text-green-200">{{ session('status') }}</div>
            @endif

            @if (($saldoAFavorUsd ?? 0) > 0)
            <div class="overflow-hidden bg-white text-sm shadow-sm dark:bg-gray-800 sm:rounded-md">
                <div class="border-b border-gray-200 px-4 py-3 font-medium dark:border-gray-600">Saldo a favor</div>
                <div class="p-4 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                    ...
                    <div class="flex gap-2">
                        <a href="{{ route('cobranza.saldo-favor.create', $cliente) }}"><x-primary-button class="!rounded-sm" type="button">Aplicar a facturas</x-primary-button></a>
                    </div>
                </div>
            </div>
            @endif
            ...
            <div class="overflow-hidden bg-white text-sm shadow-sm dark:bg-gray-800 sm:rounded-md">
                <div class="border-b border-gray-200 px-4 py-3 font-medium dark:border-gray-600">Facturas con saldo</div>
                @if ($facturas->isEmpty())
                ...
                <form method="post" action="{{ route('cobranza.cliente.pagos.store', $cliente) }}" enctype="multipart/form-data" class="space-y-0">
                        ...
                        <table class="min-w-full text-left">
                            ...
                            <tbody class="divide-y divide-gray-200 dark:divide-gray-600">
                                ...
                                <tr id="factura-cobranza-{{ $f->id }}" x-data="{ incluir: @json($incluirIni) }" @class([ 'text-gray-900 dark:text-gray-100' , 'ring-2 ring-inset ring-millennium-dark/40 dark:ring-millennium-sand/60 bg-millennium-sand/10 dark:bg-millennium-dark/20'=> $destacar,
                                    ...
                                    <td class="px-3 py-2">
                                        <span @class([ 'rounded-sm px-2 py-0.5 text-xs' , 'bg-red-100 text-red-800 dark:bg-red-900/40 dark:text-red-200'=> $ec === \App\Models\Factura::CARTERA_VENCIDA,
                                    ...
                                    <td class="px-3 py-2 text-end">
                                        <input x-ref="abono" type="text" name="abonos[{{ $f->id }}]" value="{{ old('abonos.'.$f->id) }}" inputmode="decimal" placeholder="0.00" :disabled="!incluir" title="Marcá «Abonar» en esta fila para poder escribir el monto" class="min-w-[6.5rem] w-32 rounded-sm border-gray-300 text-end text-sm shadow-sm disabled:opacity-50 dark:border-gray-700 dark:bg-gray-900 dark:text-gray-300" @input="window.millenniumOnInputMontoUsdField($event.target)" @blur="window.millenniumOnBlurMontoUsdField($event.target)" />
                                    </td>
                    ...
                    <div
                        ...
                        <div class="sm:col-span-2">
                            ...
                            <select id="metodo_pago" name="metodo_pago" x-model="metodo" class="mt-1 block w-full rounded-sm border-gray-300 shadow-sm focus:border-millennium-sand focus:ring-millennium-sand dark:border-gray-700 dark:bg-gray-900 dark:text-gray-300" required>
                        ...
                        <div x-show="grupo() === 'transferencia' || grupo() === 'usdt'" x-cloak class="rounded-sm border border-slate-200 bg-slate-50 p-3 text-sm text-slate-700 dark:border-slate-600 dark:bg-slate-800/50 dark:text-slate-200">
                            <strong>Zelle / Panamá / transferencia / USDT:</strong> cuenta destino, referencia y <strong>comprobante obligatorio</strong>.
                        </div>
                        <div x-show="grupo() === 'efectivo'" x-cloak class="rounded-sm border border-slate-200 bg-slate-50 p-3 text-sm text-slate-700 dark:border-slate-600 dark:bg-slate-800/50 dark:text-slate-200">
                        ...
                        <div class="grid sm:grid-cols-2 gap-4">
                            <div>
                                ...
                                <x-text-input id="fecha_recibo" name="fecha_recibo" type="text" inputmode="numeric" autocomplete="off" placeholder="AAAA-MM-DD" class="!rounded-sm mt-1 block w-full font-mono" value="{{ old('fecha_recibo', now()->format('Y-m-d')) }}" />
                        ...
                        <div x-show="grupo() === 'efectivo'" x-cloak class="grid sm:grid-cols-2 gap-4">
                            <div class="sm:col-span-2">
                                ...
                                <x-text-input id="recibido_por" name="recibido_por" type="text" class="!rounded-sm mt-1 block w-full" value="{{ old('recibido_por') }}" placeholder="Nombre y apellido" x-bind:disabled="grupo() !== 'efectivo'" x-bind:required="grupo() === 'efectivo'" />
                        ...
                        <div x-show="permiteTasaBs()" x-cloak class="grid sm:grid-cols-2 lg:grid-cols-3 gap-4">
                            <div>
                                ...
                                <select id="tipo_tasa" name="tipo_tasa" class="mt-1 block w-full rounded-sm border-gray-300 shadow-sm dark:border-gray-700 dark:bg-gray-900 dark:text-gray-300" :disabled="!permiteTasaBs()" :required="grupo() === 'pago_movil'">
                            ...
                            <div>
                                ...
                                <x-text-input id="valor_tasa" name="valor_tasa" type="text" inputmode="decimal" class="!rounded-sm mt-1 block w-full" x-bind:value="valorTasa" @input="onValorTasaInput($event)" @blur="onValorTasaBlur($event)" x-bind:disabled="!permiteTasaBs()" x-bind:required="grupo() === 'pago_movil'" />
                            ...
                            <div>
                                ...
                                <x-text-input
                                    ...
                                    type="text"
                                    inputmode="text"
                                    class="!rounded-sm mt-1 block w-full"
                            ...
                            <div class="sm:col-span-3 rounded-sm border border-gray-200 bg-white/80 px-3 py-2 text-sm dark:border-gray-600 dark:bg-gray-900/40">
                        ...
                        <div x-show="grupo() === 'transferencia' || grupo() === 'usdt' || grupo() === 'pago_movil'" x-cloak class="grid sm:grid-cols-2 gap-4">
                            <div class="sm:col-span-2">
                                ...
                                <x-text-input id="referencia" name="referencia" type="text" class="!rounded-sm mt-1 block w-full" value="{{ old('referencia') }}" x-bind:disabled="grupo() === 'efectivo'" x-bind:required="grupo() === 'transferencia' || grupo() === 'usdt' || grupo() === 'pago_movil'" />
                                <x-input-error :messages="$errors->get('referencia')" class="mt-2" />
                            </div>
                            <div class="sm:col-span-2">
                                <x-input-label for="banco_destino" value="Banco" />
                                <select id="banco_destino" name="banco_destino" class="mt-1 block w-full rounded-sm border-gray-300 shadow-sm dark:border-gray-700 dark:bg-gray-900 dark:text-gray-300" x-bind:disabled="grupo() === 'efectivo'" x-bind:required="grupo() === 'pago_movil'">
                        ...
                        <div x-show="grupo() === 'transferencia' || grupo() === 'usdt' || grupo() === 'efectivo' || grupo() === 'pago_movil'" x-cloak class="sm:col-span-2">
                            ...
                            <input id="comprobante"
                                ...
                                accept="image/*,.pdf"
                                capture="environment"
                                class="mt-1 block w-full text-sm text-gray-600 dark:text-gray-300 file:mr-4 file:rounded-sm file:border-0 file:bg-millennium-sand/25 file:px-3 file:py-1.5 file:text-sm file:font-medium file:text-millennium-dark dark:file:bg-millennium-dark/40 dark:file:text-millennium-sand"
                        ...
                        <div class="sm:col-span-2">
                            ...
                            <textarea id="notas" name="notas" rows="2" class="mt-1 block w-full rounded-sm border-gray-300 shadow-sm dark:border-gray-700 dark:bg-gray-900 dark:text-gray-300">{{ old('notas') }}</textarea>
                            <x-input-error :messages="$errors->get('notas')" class="mt-2" />
                        </div>

                        <x-input-error :messages="$errors->get('abonos')" class="mt-2" />
                        <div class="flex flex-wrap gap-3">
                            <x-primary-button class="!rounded-sm" type="submit">Registrar abonos</x-primary-button>
                        </div>
                    </div>
                </form>
                @endif
            ...
            <div class="border border-millennium-dark/10 bg-white p-4 text-sm shadow-sm dark:border-gray-700 dark:bg-gray-800 sm:rounded-md sm:p-6">
                ...
                <form id="form-movimientos-pdf" method="get" action="{{ route('cobranza.cliente.movimientos-pago.pdf', $cliente) }}" target="_blank" class="space-y-4" onsubmit="if (!this.querySelectorAll('input[name=\'facturas[]\']:checked').length) { alert('Seleccioná al menos una factura.'); return false; } return true;">
                    <div class="max-h-56 overflow-y-auto overflow-x-auto rounded-sm border border-gray-200 dark:border-gray-600">
                    ...
                    <div class="grid sm:grid-cols-2 gap-3 max-w-md">
                        <div>
                            ...
                            <x-text-input id="mov_desde" name="desde" type="date" class="!rounded-sm mt-1 block w-full" />
                        </div>
                        <div>
                            <x-input-label for="mov_hasta" value="Hasta (fecha recibo abono)" />
                            <x-text-input id="mov_hasta" name="hasta" type="date" class="!rounded-sm mt-1 block w-full" />
                        </div>
                    </div>
                    <x-primary-button class="!rounded-sm" type="submit">Descargar PDF movimientos</x-primary-button>
                </form>
            </div>
            @endif
