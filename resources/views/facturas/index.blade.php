    ...
    <div class="py-6">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8 space-y-4">
            ...
            <p class="text-xs text-gray-600 dark:text-gray-400">
                ...
                Pestaña <strong>Vigentes</strong>: facturas por pagar; <strong>Canceladas</strong>: historial pagado; <strong>Todas</strong>: combiná con el filtro de estado pago.
                Con saldo: <strong>Registrar pago</strong> / <strong>Cobranza conjunta</strong> (casillas mismo cliente).
            </p>
            @else
            <p class="text-xs text-gray-600 dark:text-gray-400">
                Solo ves las facturas donde vos sos el <strong>vendedor de la factura</strong>; podés filtrar por cliente y <strong>verificar precios</strong> en el detalle cuando corresponda.
                Pestañas <strong>Vigentes</strong>, <strong>Canceladas (historial)</strong> y <strong>Todas</strong>.
            </p>
            @endif

            <div class="bg-white dark:bg-gray-800 shadow-sm sm:rounded-lg overflow-hidden text-sm" @if($puedeOperativa) x-data="facturasCobranzaSeleccion({ baseUrl: @js(url('/cobranza/cliente')) })" x-ref="cobWrap" @endif>
                @if ($puedeOperativa)
                    ...
                    <table class="min-w-full text-left">
                        <thead class="bg-gray-50 dark:bg-gray-700 text-gray-700 dark:text-gray-200">
                            <tr>
                                ...
                                <th class="px-3 py-2 font-medium" title="Verificación">Verif.</th>
                            ...
                            <tr class="text-gray-900 dark:text-gray-100">
                                ...
                                <td class="px-3 py-2">
                                    ...
                                    @else
                                    <span class="px-2 py-0.5 rounded text-xs bg-slate-200 text-slate-800 dark:bg-slate-600 dark:text-slate-100">Factura por pagar</span>
                                    @endif
                                </td>
                                <td class="px-3 py-2">
                                ...
                                <td class="px-3 py-2 text-end whitespace-nowrap">
                                    ...
                                    @endcan
                                    @if ($puedeOperativa && $cobrable)
                                    <a href="{{ route('cobranza.cliente', $f->cliente) }}?destacar={{ $f->id }}" class="text-millennium-dark dark:text-millennium-sand hover:underline ms-2 font-medium whitespace-nowrap" title="Registrar pago y distribuirlo entre facturas de este cliente">Registrar pago</a>
                                    @endif
                                </td>
                            </tr>
                            @empty
                            <tr>
                                <td colspan="{{ $puedeOperativa ? 11 : 10 }}" class="px-4 py-8 text-center text-gray-500">No hay facturas.</td>
                            </tr>
                            @endforelse
                        </tbody>
                    </table>
